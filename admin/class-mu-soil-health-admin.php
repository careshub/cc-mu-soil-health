 ...
 * functionality, then refer to `public/class-mu-soil-health.php`
 *
 * @package   Community_Commons_MU_Soil_Health_Admin
 * @author  Chris Barnett
 */
class CC_MU_Soil_Health_Admin extends CC_MU_Soil_Health_Public {
	...
	public function add_plugin_admin_menu() {
		...
		$this->plugin_screen_hook_suffix = add_options_page(
			__( 'MU_Soil_Health', 'mu-soil-health' ),
			__( 'MU_Soil_Health', 'mu-soil-health' ),
			'manage_options',
			$this->plugin_slug,
	...
	public function add_action_links( $links ) {
		...
		return array_merge(
			array(
				'settings' => '<a href="' . admin_url( 'options-general.php?page=' . $this->plugin_slug ) . '">' . __( 'Settings', 'mu-soil-health' ) . '</a>'
			),
			$links
		);
	...
	/**
	 * Tell CC Stats that this plugin is associated with this hub.
	 *
	 * @since 1.0.0
	 */
	public function report_custom_plugin( $custom_plugins, $group_id ) {
		if ( $group_id == mu_soil_health_get_group_id() ) {
			$custom_plugins[] = "CC MU_Soil_Health";
		}
